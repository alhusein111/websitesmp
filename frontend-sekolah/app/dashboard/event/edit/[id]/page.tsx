...
export default function EditEventPage() {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL || "http://localhost:1337"}/api/events/${eventId}`, {
        ...
        if (attrs) {
          setNamaKegiatan(attrs.NamaKegiatan || "");
          setTanggalMulai(attrs.TanggalMulai || "");
          setTanggalSelesai(attrs.TanggalSelesai || "");
          setLokasi(attrs.Lokasi || "");
          setDeskripsiSingkat(attrs.DeskripsiSingkat || "");
          setTampilkanDiSidebar(attrs.TampilkanDiSidebar || false);
        }
      } catch (error) {
        console.error(error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const jwtToken = (session as any)?.jwt;
    if (!jwtToken) {
      toast.error("Gagal: Akses ditolak.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Berhasil! Kegiatan telah diperbarui.", { id: toastId });
        setTimeout(() => {
  ...
  return (
    <div className="max-w-4xl pb-12">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold">Edit Agenda Kegiatan</h2>
          <p className="text-gray-500 text-sm">Perbarui informasi acara yang sudah ada.</p>
        </div>
      </div>
